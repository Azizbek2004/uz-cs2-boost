...
export default function SprayCanvas({ onScoreUpdate }: SprayCanvasProps) {
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (sessionComplete) {
      const accuracy =
        pattern.length > 0
          ? Math.round((score / (pattern.length * 100)) * 100)
          : 0;
      ...
      ctx.fillText(
        `Score: ${score} / ${pattern.length * 100}`,
        centerX,
        centerY + 10,
      );
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const bulletScore = isClose
      ? Math.max(0, Math.round(100 - distance * 2))
      : 0;
    ...
    <div>
      ...
      <AnimatePresence>
        {isTraining && !sessionComplete && (
          ...
          >
            💡 Click where the next bullet should land. Green circle = target
            position.
            {selectedWeapon === "AK-47" &&
              " The AK-47 pulls up then sweeps left-right."}
            {selectedWeapon === "M4A4" &&
              " The M4A4 has a more vertical pattern."}
            {selectedWeapon === "M4A1-S" &&
              " The M4A1-S has a tight, controlled spray."}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
